classdef LSBP
    ...
    methods
        function self = LSBP(self,q0,qf,tf,V)
            temp_tb = zeros(4:1);
        ...
        function [q,qdot] = curr_increment(self,t)
            ...
            for i = 1:4
                if t<=self.tb(i)
                    q(i) = self.q0(i) + self.alpha/2*power(t,2);
                    qdot(i) = self.alpha*t;
                ...
                elseif t <= self.tf - self.tb(i)
                    ...
                    q(i) = self.qf(i) - self.alpha/2*power(t,2) - self.alpha/2*power(self.tf,2) + self.alpha*self.tf*t;
                    qdot(i) = self.V-self.alpha*t;
                end
            end
        end
